risk_pred_core <- function(age, sex, eGFR, uACR, is_north_american,
  ...
  if (six) {
    ...
    dm_factor <- -0.1475; htn_factor <- 0.1426
  } else if (eight) {
    ...
    albumin_factor <- -0.3441; phosph_factor <- +0.2604; bicarb_factor <- -0.07354; calcium_factor <- -0.2228
  } else {
...
RiskPredictor <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(df=NULL, columns=NULL) { self$df <- df; self$columns <- columns },
    predict_kfre = function(years, is_north_american, use_extra_vars=FALSE, num_vars=4, precision=NULL) {
      cols <- self$columns; d <- self$df
