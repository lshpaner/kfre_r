...
.ap_like_sklearn <- function(y_true, y_prob) {
  if (length(y_true) == 0L) return(NA_real_)
  y <- as.integer(y_true > 0)
  n_pos <- sum(y == 1L)
  if (n_pos == 0L) return(0)
...
plot_kfre_metrics <- function(df,
  ...
  if (length(missing) > 0L) {
    stop(sprintf("Must derive KFRE probabilities before generating performance metrics. Missing: %s",
  ...
  preds <- list()
  for (n in as.integer(num_vars)) {
    preds[[sprintf("%dvar", n)]] <- lapply(as.integer(show_years),
                                           function(yr) df[[sprintf("kfre_%dvar_%dyear", n, yr)]])
  }
  result <- list(y_true = y_true, preds = preds, outcomes = outcomes)
  
  # honor mode = "prep"
  if (identical(mode, "prep")) return(result)
  
  # ---- helpers ----
  auc_fast <- function(yt, yp) {
    o <- order(yp, decreasing = TRUE); y <- as.integer(yt[o] > 0)
  ...
  ap_like_sklearn <- function(yt, yp) {
    if (length(yt) == 0L) return(NA_real_)
    y <- as.integer(yt > 0); n_pos <- sum(y == 1L); if (n_pos == 0L) return(0)
    o <- order(yp, decreasing = TRUE); y <- y[o]; s <- yp[o]
    uniq <- sort(unique(s), decreasing = TRUE); key <- match(s, uniq)
    pos_at_t <- as.integer(tapply(y,    key, function(v) sum(v == 1L))); pos_at_t[is.na(pos_at_t)] <- 0L
    neg_at_t <- as.integer(tapply(1L-y, key, function(v) sum(v == 1L))); neg_at_t[is.na(neg_at_t)] <- 0L
    tp <- cumsum(pos_at_t); fp <- cumsum(neg_at_t)
    precision <- tp / pmax(tp + fp, 1L); recall <- tp / n_pos
  ...
  make_roc_df <- function(curr_nums) {
    ...
    for (n in as.integer(curr_nums)) {
      ...
      for (i in seq_along(y_true)) {
        ...
        o <- order(yp, decreasing = TRUE); y <- yt[o]
        tps <- cumsum(y == 1L); fps <- cumsum(y == 0L)
        tpr <- tps / max(1L, sum(y == 1L))
        fpr <- fps / max(1L, sum(y == 0L))
        lbl <- sprintf("%d-variable %s outcome (AUC = %.*f)",
  ...
  make_pr_df <- function(curr_nums) {
    ...
    for (n in as.integer(curr_nums)) {
      ...
      for (i in seq_along(y_true)) {
        ...
        precision <- c(1, precision); recall <- c(0, recall)
        lbl <- sprintf("%d-variable %s outcome (AP = %.*f)",
  ...
  color_scale_for <- function(n_items) {
    ggplot2::scale_color_manual(values = grDevices::hcl.colors(n_items, palette = "Dark2"),
      ...
      if (plot_type %in% c("auc_roc", "all_plots")) {
        ...
        if (isTRUE(save_plots)) {
          if (!is.null(image_path_png)) {
            ...
            ggplot2::ggsave(filename = file.path(image_path_png,
                                                 sprintf("%s_auc_roc_curve_combined.png",
          ...
          if (!is.null(image_path_svg)) {
            ...
            ggplot2::ggsave(filename = file.path(image_path_svg,
                                                 sprintf("%s_auc_roc_curve_combined.svg",
                                                         if (is.null(image_prefix)) "auc_roc_curve_combined" else image_prefix)),
      ...
      if (plot_type %in% c("precision_recall", "all_plots")) {
        ...
        if (isTRUE(save_plots)) {
          if (!is.null(image_path_png)) {
            dir.create(image_path_png, recursive = TRUE, showWarnings = FALSE)
            ggplot2::ggsave(filename = file.path(image_path_png,
                                                 sprintf("%s_pr_curve_combined.png",
          ...
          if (!is.null(image_path_svg)) {
            dir.create(image_path_svg, recursive = TRUE, showWarnings = FALSE)
            ggplot2::ggsave(filename = file.path(image_path_svg,
                                                 sprintf("%s_pr_curve_combined.svg",
                                                         if (is.null(image_prefix)) "pr_curve_combined" else image_prefix)),
        ...
        if (plot_type %in% c("precision_recall", "all_plots")) {
          ...
          if (isTRUE(save_plots)) {
            if (!is.null(image_path_png)) {
              dir.create(image_path_png, recursive = TRUE, showWarnings = FALSE)
              ggplot2::ggsave(filename = file.path(image_path_png,
                                                   sprintf("%s_%dvar_precision_recall.png",
                                                           if (is.null(image_prefix)) as.character(n) else image_prefix, n)),
                              plot = p_pr, width = fig_size[1], height = fig_size[2], units = "in", dpi = 300)
            }
            if (!is.null(image_path_svg)) {
              dir.create(image_path_svg, recursive = TRUE, showWarnings = FALSE)
              ggplot2::ggsave(filename = file.path(image_path_svg,
                                                   sprintf("%s_%dvar_precision_recall.svg",
                                                           if (is.null(image_prefix)) as.character(n) else image_prefix, n)),
                              plot = p_pr, width = fig_size[1], height = fig_size[2], units = "in")
            }
          }
...
if (identical(mode, "both")) return(result)
invisible(NULL)
}
